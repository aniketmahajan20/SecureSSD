...
class persecond:
    def __init__(self):
        ...
        self.time = 0
        self.m = []
        self.avg_write_length_ps = []
        
        #function for managing incoming requests
    def new_entry(self,packet):
        if packet[0] > self.time:
            self.time = self.time + 1
            self.avg_write_length_ps.append(self.add_length()/(len(self.length)+1))
            self.length.clear()
            ...
            while self.time != packet[0]:
                self.rps.append(0)
                self.wps.append(0)
                self.avg_write_length_ps.append(0)
                self.time = self.time + 1
                self.m.append(self.time)
...
#%% plotting average write length per second
plt.plot(ani.m,ani.avg_write_length_ps)
plt.title("Average write length")
plt.xlabel("Time in sec.")
plt.ylabel("Length")  