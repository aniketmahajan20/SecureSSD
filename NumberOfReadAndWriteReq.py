...
with open('/Users/apple/Desktop/Research/Dr. NEERAJ GOEL/Logs/WannaCry3_erps.csv', 'r') as csvfile:
    # creating a csv reader object
    csvreader = csv.reader(csvfile)
...
class persecond:
    ...
    def new_entry(self,packet):
        if packet[0] > self.time:
            ...
            self.rrps.append(self.rr)
            self.wrps.append(self.wr)
            self.delete()
...
#%% Plotting read requests per second
plt.plot(ani.m,ani.rrps)
plt.title("Read Requests per second")
plt.xlabel("Time in sec.")
plt.ylabel("Number of read requests")  
#%% Plotting write requests per second
plt.plot(ani.m,ani.wrps)
plt.title("Write Requests per second")
plt.xlabel("Time in sec.")
plt.ylabel("Number of Write requests") 
#%% Plotting Erasure Req Per second 
plt.plot(ani.m,erps)
plt.title("Erasure Requests per second")
plt.xlabel("Time in sec.")
plt.ylabel("Number of Erasure requests") 
#%% Plotting Fraction of Erasure over total write request per second
new_wrps = []
for i in range(len(ani.wrps)):
    if ani.wrps[i]==0:
        new_wrps.append(1)
    else:
        new_wrps.append(ani.wrps[i])
erps_over_write = list(map(truediv, erps, new_wrps))
plt.plot(ani.m, erps_over_write)
plt.title("Erasure Requests over write per second")
plt.xlabel("Time in sec.")